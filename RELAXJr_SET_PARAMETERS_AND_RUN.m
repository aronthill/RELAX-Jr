...
%-------------------------------------------------------------------------%
% This script can be used as an alternative to the EEGLAB RELAX-Jr GUI to
% process EEG data for those who would prefer to take a script-based
% approach.

% This pipeline firstly filters your data, then deletes bad electrodes and
% marks extremely bad periods for exclusion from further processing. It
% then creates a mask of clean and artifact data specific to the length of
% your data file, which is submitted to a multiple weiner filter (MWF) in
% order to clean your data. The MWF cleaned data is then submitted to an
% independent component analysis, and artifactual components detected by
% Adjusted ADJUST are cleaned by wavelet enhanced ICA. The resulting saved
% files are continuous, very well cleaned of artifacts, while still
% preserving the neural signal. Note that the pipeline will only work on
% continuous data.
%-------------------------------------------------------------------------%
...
% formatted (.set format), and specify this folder location in
% RELAX_cfg.myPath (these files should have triggers recoded to reflect
% participant accuracy if desirable, and not have been re-referenced or
% have any other pre-processing completed yet).

% 3) Specify the parameters in RELAX_cfg. These parameters let you
% determine a range of factors that influence the mask creation, eg. "how
% many epochs can show muscle activity in a channel before that channel is
...
% "should I run the MWF twice or just once", and "what is the threshold of
% artifact severity,  above which sections are marked as an artifact, below
% which they are not?". Additionally, particular artifact detection and
% masking functions could be excluded if you're not concerned about those
% artifacts by commenting that function out (adding a % symbol at the start
% of the line). For example, you might only want to include muscle, and
% drift artifacts in the MWF cleaning, and not blink and horizontal eye
% movements if you are analysing eyes closed resting data.

% 4)Click "Run" in the menu up the top of this script The script will then
% process all participants, taking ~10 min per participant. You can run
% just a subset of the participants first by altering the following line:
% "for FileNumber=1:numel(RELAX_cfg.files)"

% 5) The script will produce files that contain cleaning quality values for
% relevant processes in this script. Means, SD's (or medians and MADs) and
% ranges across all participants can be examined to determine the success
% of the script for the study as a whole, and to identify potential issues
% with specific files (eg. the artifact to residue ratio is ~1, suggesting
% artifacts weren't removed successfully, or >75% of the epochs contained
% muscle activity, suggesting bad data throughout). These statistics should
...
% minimum length of a marked patch of clean or artifact data (if patches
% are too short, the MWF can't create an adequate mask. The amount of data
% marked as artifact around eye blinks can particularly affect this).

% 6) The script will produce output EEG files that have been cleaned into a
% folder labelled 1xMWF for the first run, 2xMWF, and 3xMWF (if you select
% to save these). Cleaned data will be saved into the Cleaned_data folder.
% The cleaned files will have electrodes removed, and are referenced to the
% average reference. From this point, I would interpolate missing
% electrodes, re-code the task related triggers as to whether the
% participant was correct or not, epoch the data, reject epochs that still
% show artifacts (the epoch rejection wrapper could be adapted to achieve
% this) and separate different conditions into different files in
% preparation for analysis.

% Note that if you have very large files (or your computer doesn't have
% much RAM), the MWF cleaning may run out of RAM (and suggest a created
% array is too large). This could be addressed by downsampling the data, or
% reducing RELAX_cfg.MWFDelayPeriod=8; to 4-5. This will reduce the number
% of samples that the MWF cleaning takes into account when characterising
% artifacts, but still cleans very effectively.

% Note that the pipeline does not work well on MATLAB versions <2016, we
% suggest updating to a more recent version of MATLAB if your version is
% pre-2016.
...
%% DEPENDENCIES
%-------------------------------------------------------------------------%
% Toolboxes you need to install, and cite if you use this script:
% use fileseparators 'filesep' for increased compatability if necessary (replace the '/' [mac] or '\' [windows] with a filesep [outside of quotes]) 
...
addpath('/Users/aronhill/Documents/MATLAB/eeglab2023.0');
eeglab;

% *PREP PIPELINE*
% Required to reject bad electrodes (install plugin to EEGLAB, or via the github into the EEGLAB plugin folder): 
...
%addpath('/Users/aronhill/Documents/MATLAB/fieldtrip-20210525');
addpath('/Users/aronhill/Documents/MATLAB/fieldtrip-20230716');

% *FAST ICA*
% http://research.ics.aalto.fi/ica/fastica/code/dlcode.shtml 
addpath('/Users/aronhill/Documents/MATLAB/eeglab2023.0/plugins/FastICA_25/');

% *Adjusted-ADJUST*
addpath('/Users/aronhill/Documents/MATLAB/EXTERNAL_PLUGINS/MADE/original_files/adjusted_adjust_scripts');

% *Adjust*
addpath('/Users/aronhill/Documents/MATLAB/eeglab2023.0/plugins/ADJUST1.1.1')
...
addpath(genpath('/Users/aronhill/Documents/MATLAB/eeglab2023.0/plugins/mwf-artifact-removal-master'));

% *Specify  RELAX folder location (this toolbox)*
addpath('/Users/aronhill/Documents/MATLAB/eeglab2023.0/plugins/RELAXJr/'); 

% *Specify your electrode locations with the correct cap file*
RELAX_cfg.caploc= []; %'/Users/aronhill/Documents/MATLAB/cap_location_files/GSN-HydroCel-65.sfp'; % path containing electrode positions. Set to =[] if electrode locations are already in your EEG file.

% *LOCATION OF THE DATA YOU WISH TO PROCESS (.set files)*
RELAX_cfg.myPath='/Volumes/LaCie_5TB/RELAXJr_test_data/EO/';
...
RELAX_cfg.Perform_wICA_on_adjusted_ADJUST=1; % 1 = Perform wICA on artifact components marked by adjusted ADJUST (1 for yes, 0 for no).
RELAX_cfg.Perform_ICA_subtract=0;            % 1 = Perform ICA subtract on artifact components marked by Adjusted ADJUST (1 for yes, 0 for no) (non-optimal, intended to be optionally used separately to wICA rather than additionally)
...
%***NOTE:*** the above electrodes use the 10-10 naming convention. Please check
%to ensure that these match the electrode names for your files! If using a
%Geodesic Sensor Net, naming might be different (e.g., 'E1', 'E2', etc), in
%which case blink and HEOG electrodes would need to be defined using this
...
RELAX_cfg.ElectrodesToDelete={'EOG1';'EOG2';'EOG3';'EOG4';'CB1'; 'CB2'; 'HEOG'; 'IO1'; 'M1'; 'M2'; 'LO1'; 'LO2'; 'E1'; 'E3'; 'ECG'; 'SO1'; 'ECG'; 'SPARE1'; 'SPARE2'; 'SPARE3'; 'BP1'; 'BP2'; 'VEOG'};
...
% The RELAX-Jr cleaning pipeline does not need eye, heart, or mastoid electrodes for effective cleaning.
...
RELAX_cfg.MWFDelayPeriod=30; % The MWF includes both spatial and temporal information when filtering out artifacts. Longer delays apparently improve performance. 

%% Check for dependencies
%-------------------------------------------------------------------------%

eeglabPath = fileparts(which('eeglab'));
MWFPluginPath=strcat(eeglabPath, [filesep 'plugins' filesep 'mwf-artifact-removal-master' filesep]);
...
PluginPath=strcat(eeglabPath, [filesep 'plugins' filesep]);
...
%% RUN SCRIPT BELOW:
RELAX_cfg.FilesToProcess=1:1;   %(size(RELAX_cfg.files,2));%:numel(RELAX_cfg.files); % Set which files to process
...
[RELAX_cfg, FileNumber, CleanedMetrics, RawMetrics, RELAXProcessingRoundOneAllParticipants, RELAXProcessingRoundTwoAllParticipants, RELAXProcessing_wICA_AllParticipants,...
        RELAXProcessing_ICA_AllParticipants, RELAXProcessingRoundThreeAllParticipants, RELAX_issues_to_check, RELAXProcessingExtremeRejectionsAllParticipants] = RELAXJr_Wrapper (RELAX_cfg); %RELAX_Wrapper_child_EEG (RELAX_cfg);
