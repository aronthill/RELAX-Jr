...
function [OutlierParticipantsToManuallyCheck,EpochRejections,RELAX_epoching_cfg] = pop_RELAXJr_epoch_the_clean_data(RELAX_epoching_cfg)
...
if ~isfield(RELAX_epoching_cfg,'CleanedPath')
    RELAX_epoching_cfg.CleanedPath = '/Volumes/LaCie_5TB/RELAXJr_test_data/EO/RELAXProcessed/Cleaned_Data';
end
...
%Set at 100 by default as we have found that occasionally using lower
%values (e.g., 60) might still catch high-amplitude alpha activity in young
%subjects. 
if ~isfield(RELAX_epoching_cfg,'reject_amp')
    RELAX_epoching_cfg.reject_amp=100; % Absolute voltage amplitude threshold - if an epoch has voltages that deviate from 0 by more than this value the epoch is rejected
end
...
if ~isfield(RELAX_epoching_cfg,'DataType')
    RELAX_epoching_cfg.DataType='Resting'; % 'Task' for cognitive tasks, 'Resting' for data without stimuli presented
end
...
if ~isfield(RELAX_epoching_cfg,'restingdatatriggerinterval')
    RELAX_epoching_cfg.restingdatatriggerinterval=3.0; % (seconds) sets how often an 'X' trigger is inserted into resting data for epoching. 3.5 with [-2.5 2.5] PeriodToEpoch provides 5s epochs with 1.5s overlaps
end

if ~isfield(RELAX_epoching_cfg,'TriggersToEpoch')
    RELAX_epoching_cfg.TriggersToEpoch={'Trigger_A' 'Trigger_B' 'Tirgger_C' 'Trigger_D'}; % triggers to use for epoching
end

if ~isfield(RELAX_epoching_cfg,'PeriodToEpoch')
    RELAX_epoching_cfg.PeriodToEpoch=[-1.5 1.5]; % [start end] surrounding trigger
end
...
if ~isfield(RELAX_epoching_cfg,'BL_correction_method')
    RELAX_epoching_cfg.BL_correction_method='none'; % Set type of baseline correction to perform ('regression' = recommended, 'subtraction' = traditional but not recommended)
end

if ~isfield(RELAX_epoching_cfg,'BL_correction_Factor_1_Level_1')
    RELAX_epoching_cfg.BL_correction_Factor_1_Level_1={'Trigger_A' 'Trigger_C' }; % triggers to include in factor 1's level 1 for regression BL correction (all other triggers are included in factor 1's level 2)
end

if ~isfield(RELAX_epoching_cfg,'BL_correction_Factor_2_Level_1')
    RELAX_epoching_cfg.BL_correction_Factor_2_Level_1={'Trigger_A' 'Trigger_B' }; % triggers to include in factor 2's level 1 for regression BL correction (all other triggers are included in factor 2's level 2)
end
...
uilist = {{'style', 'text', 'string', 'Cleaned data folder:','fontweight','bold','fontsize', 9} ...
          ...
          {'style', 'popupmenu', 'string', TaskOrRestOptions, 'tag', 'TaskOrRestOpts','Value',2,'fontsize', 9} ...
          ...
          {'style', 'popupmenu', 'string', BLCorrectionOptions, 'tag', 'BLCorrectionOpts','Value',3,'fontsize', 9} ...
...
result = inputgui('geometry', geometry, 'geomvert', [1 .4 1 .4 1 0.4 1 0.4 1 0.4 1 0.4 1 0.4 1 0.4 1 0.4 1 0.4 1 0.4 1 0.4 1],  'uilist', uilist, 'title', 'RELAX-Jr Parameter Setting',  'helpcom', 'pophelp(''pop_RELAXJr'')');
