...
%% pop_RELAXJr:
% Clean data with RELAX-Jr via the EEGLAB gui:
function [RELAX_cfg, FileNumber, CleanedMetrics, RawMetrics, RELAXProcessingRoundOneAllParticipants, RELAXProcessingRoundTwoAllParticipants, RELAXProcessing_wICA_AllParticipants,...
        RELAXProcessing_ICA_AllParticipants, RELAXProcessingRoundThreeAllParticipants, RELAX_issues_to_check, RELAXProcessingExtremeRejectionsAllParticipants] = pop_RELAXJr(RELAX_cfg)
...
% Adjusted-ADJUST
% Leach SC, Morales S, Bowers ME, et al. Adjusting ADJUST: Optimizing the ADJUST algorithm for pediatric data using geodesic nets. Psychophysiology. 2020;57:e13566.
% https://github.com/ChildDevLab/MADE-EEG-preprocessing-pipeline/tree/master/adjusted_adjust_scripts

% PICARD (Preconditioned ICA for Real Data). 
% Pierre Ablin, Jean-François Cardoso, Alexandre Gramfort. Faster independent component analysis by preconditioning with Hessian approximations. IEEE Transactions on Signal Processing, 2018, 66, 4040-4049. 
% Pierre Ablin, Jean-François Cardoso, Alexandre Gramfort. Faster ICA under orthogonal constraint. ICASSP, 2018. 
% https://github.com/pierreablin/picard/tree/master
...
if ~isfield(RELAX_cfg,'caploc')
    RELAX_cfg.caploc= []; %'/Users/aronhill/Documents/MATLAB/cap_location_files/GSN-HydroCel-65.sfp'; %path containing electrode positions
end
...
% However, the included default options appear to perform well on out trial data.
...
if ~isfield(RELAX_cfg,'Perform_wICA_on_adjusted_ADJUST')
...
if ~isfield(RELAX_cfg,'Report_all_ICA_info')
    RELAX_cfg.Report_all_ICA_info='no';  % set to 'yes' to provide detailed report of Adjusted ADJUST artifact information. Runs ~20s slower per file.
end
...
%ATH: Added to RELAX-Jr from beta feature in RELAX
...
% 0 = e.g., task-related data where participants are focused with eyes open
% 1 = e.g., eyes-closed recordings, but with participants who might still open their eyes at times
% 2 = e.g., eyes-closed resting with highly compliant participants and recordings that were strictly made only when participants had their eyes closed.
...
%note that we have provided electrode lables in 10-10 nomenculature,
%however, many users will likely have electrodes that follow the Geodesic
%Sensor Net naming conventions (e.g., 'E' followed by electrode no. [E1,
%E2, E3 etc]). Please be sure to check your specific electrode labels prior
%to defining the electrodes here. 
...
icaOptions = {'picard', 'fastica_symm','fastica_defl','extended_infomax_ICA','cudaica','amica'};
...
%ATH NOTE: Additional options for 6Hz low pass option and Zapline now added
...
geometry = {[0.5 1.0 0.2 1.0 1.0 0.2] ... % setting directory / file to process
            ...
            1 ...
            [5 .8 .5 2 0.5 2 1.2] ... % bandpass filter settings  ***ATH added Zapline option for cleaning line noise from RELAX beta***
            1 ...
            ...
            1 ...
            [1.3 1 0.3 0.9 1.5 1.6 1.5]... % Blink probability and electrode options  ***ATH added 6Hz low pass filter option from RELAX beta***
            1 ...
...
uilist = {{'style', 'text', 'string', 'Raw data folder:','fontweight','bold','fontsize', 9} ...
          ...
          ' ' RELAX_cfg.BlinkElectrodes{7,1} ' ' RELAX_cfg.BlinkElectrodes{8,1} ' ' RELAX_cfg.BlinkElectrodes{9,1}...
          ' ' RELAX_cfg.BlinkElectrodes{10,1} ' ' RELAX_cfg.BlinkElectrodes{11,1} ' ' RELAX_cfg.BlinkElectrodes{12,1}],'fontsize', 9} ...
          ...
          ' ' RELAX_cfg.HEOGLeftpattern{1,7} ' ' RELAX_cfg.HEOGLeftpattern{1,8} ' ' RELAX_cfg.HEOGLeftpattern{1,9} ' ' RELAX_cfg.HEOGLeftpattern{1,10}],'fontsize', 9} ...
          ...
          ' ' RELAX_cfg.HEOGRightpattern{1,7} ' ' RELAX_cfg.HEOGRightpattern{1,8} ' ' RELAX_cfg.HEOGRightpattern{1,9} ' ' RELAX_cfg.HEOGRightpattern{1,10}],'fontsize', 9} ...
          {}...
          ...
          {'style', 'popupmenu', 'string', YesNoOptions, 'tag', 'YesNoOpts','Value',2,'fontsize', 9} ...
          {}...
...
result = inputgui('geometry', geometry, 'geomvert', [1 .4 1 .4 1 0.4 1 0.4 1 0.4 1 0.4 1 0.4 1 0.4 1 0.4 1 0.4 1 0.4 1 0.4 1 0.4 1 0.4 1 0.4 1],  'uilist', uilist, 'title', 'RELAX-Jr Parameter Settings',  'helpcom', 'pophelp(''pop_RELAXJr'')');
...
RELAX_cfg.NotchFilterType=LineNoiseOptions{result{7}}; %ATH
RELAX_cfg.MaxProportionOfElectrodesThatCanBeDeleted=str2double(result{8});
RELAX_cfg.ProportionOfExtremeNoiseAboveWhichToRejectChannel=str2double(result{9});
RELAX_cfg.ProportionOfMuscleContaminatedEpochsAboveWhichToRejectChannel=str2double(result{10});
RELAX_cfg.ExtremeAbsoluteVoltageThreshold=str2double(result{11});
RELAX_cfg.ExtremeImprobableVoltageDistributionThreshold=str2double(result{12});
RELAX_cfg.ExtremeSingleChannelKurtosisThreshold=str2double(result{13});
RELAX_cfg.ExtremeAllChannelKurtosisThreshold=str2double(result{14});
RELAX_cfg.ExtremeVoltageShiftThreshold=str2double(result{15});
RELAX_cfg.ExtremeBlinkShiftThreshold=str2double(result{16});
RELAX_cfg.ExtremeDriftSlopeThreshold=str2double(result{17});
RELAX_cfg.MWFDelayPeriod=str2double(result{18});
RELAX_cfg.Do_MWF_Once=result{19};  % 1 = Perform the MWF cleaning a second time (1 for yes, 0 for no).
RELAX_cfg.Do_MWF_Twice=result{20}; % 1 = Perform the MWF cleaning a second time (1 for yes, 0 for no).
RELAX_cfg.Do_MWF_Thrice=result{21};
RELAX_cfg.Perform_wICA_on_adjusted_ADJUST=result{22};
RELAX_cfg.Perform_ICA_subtract=result{22}-1;
RELAX_cfg.ICA_method = icaOptions{result{23}};
RELAX_cfg.ProbabilityDataHasNoBlinks=(result{24})-1;
RELAX_cfg.BlinkElectrodes= strtrim(strsplit(strrep(result{25},',','')))';
RELAX_cfg.LowPassFilterAt_6Hz_BeforeDetectingBlinks=YesNoOptions{result{26}}; %ATH
RELAX_cfg.HEOGLeftpattern = string(strtrim(strsplit(strrep(result{27},',',''))));
RELAX_cfg.HEOGRightpattern= string(strtrim(strsplit(strrep(result{28},',',''))));
RELAX_cfg.MuscleSlopeThreshold=str2double(result{29});
RELAX_cfg.MaxProportionOfDataCanBeMarkedAsMuscle=str2double(result{30});
RELAX_cfg.DriftSeverityThreshold=str2double(result{31});
RELAX_cfg.ProportionWorstEpochsForDrift=str2double(result{32});
RELAX_cfg.HorizontalEyeMovementThreshold=str2double(result{33});
RELAX_cfg.InterpolateRejectedElectrodesAfterCleaning=YesNoOptions{result{34}};
...
fprintf([
    ...
    '\nAn Automated Pre-Processing Pipeline for Developmental EEG Recordings Based on the RELAX Algorithm. bioRxiv.'...
    '\n----------------------------------------------------------------------------------------------------------------'
    ]);
...
if RELAX_cfg.HighPassFilter>0.5
    Warning='You have high pass filtered above 0.5 Hz, which might adversely affect ERP analyses';
    fprintf(['\n***Warning***:' Warning]); 
end
...
[RELAX_cfg, FileNumber, CleanedMetrics, RawMetrics, RELAXProcessingRoundOneAllParticipants, RELAXProcessingRoundTwoAllParticipants, RELAXProcessing_wICA_AllParticipants,...
        RELAXProcessing_ICA_AllParticipants, RELAXProcessingRoundThreeAllParticipants, RELAX_issues_to_check, RELAXProcessingExtremeRejectionsAllParticipants] = RELAXJr_Wrapper (RELAX_cfg);
   
if RELAX_cfg.HighPassFilter>0.5
    Warning='You have high pass filtered above 0.5 Hz, which can adversely affect ERP analyses';
end
